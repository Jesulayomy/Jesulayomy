    ...
    <div className='flex flex-col lg:flex-row py-4 border-b border-zinc-500 justify-between'>
      <div className='text-end lg:w-[28%]'>
        <div className='flex flex-col'>
          <span className='text-4xl text-slate-200'>{exp.company}</span>
          <span className='text-2xl text-slate-100'>{exp.role}</span>
        </div>
      </div>
        ...
        <div className='flex flex-col'>
          <div className='flex flex-col md:flex-row items-center w-[100%]'>
            <div className='flex flex-col w-[100%] md:w-[40%]'>
              <span className='text-xs text-slate-200'>{exp.type}, {exp.location}</span>
              <span className='text-xs text-slate-100'>{exp.duration}</span>
            </div>
            <div
          ...
          <ul className='list-disc list-inside'>
            {exp.tasks.map((task, index) => (
              <li key={index} className='text-xs md:text-base text-slate-200'>{task}</li>
            ))}
          </ul>
        </div>
      </div>
